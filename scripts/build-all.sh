...
# Cross-build fastermail (the `fm` binary) for all release targets using
# cargo-zigbuild.
#
# Runs on macOS or Linux:
#   macOS  — builds all targets (darwin + linux + windows)
#   Linux  — builds linux + windows targets only (darwin targets require macOS)
#
# Prerequisites:
...
#   installed). Zig 0.14 cannot resolve Apple frameworks when SDKROOT is set
#   — its sysroot doesn't understand Apple's SDK layout — so we never invoke
#   zigbuild for darwin targets and only export SDKROOT along the cargo path.
...
# (zig 0.14 can't resolve Apple frameworks through its own sysroot).
...
for target in "${build_targets[@]}"; do
  case "$target" in
    *-windows-*)
      bin="target/$target/release/fm.exe"
      out="$OUTDIR/fm-$target.exe"
      ;;
    *)
      bin="target/$target/release/fm"
      out="$OUTDIR/fm-$target"
      ;;
  esac
  if [ -f "$bin" ]; then
    cp "$bin" "$out"
...
echo ""
echo "--- Results ---"
ls -lh "$OUTDIR"/fm-* 2>/dev/null || echo "(no binaries produced)"
